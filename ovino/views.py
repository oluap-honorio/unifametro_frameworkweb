...
def index(request):
    context = {}
    context["ovinos"] = Ovino.objects.all()
...
def show(request, id):
    context = {}
    context["ovino"] = Ovino.objects.get(id=id)
    return render(request, "ovino_show.html", context)
...
def update(request, id):
    ...
    if form.is_valid():
        form.save()
        return HttpResponseRedirect("/ovino/" + id)
    context["form"] = form
